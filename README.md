...
| 8 | 🪟 **[cmux Session](./extensions/cmux-session.ts)** | Bridges Pi session lifecycle events into [cmux](https://cmux.com)'s restorable session store. Enables agent session resume when cmux restores your workspace. Installed automatically by `cmux hooks setup`. |
